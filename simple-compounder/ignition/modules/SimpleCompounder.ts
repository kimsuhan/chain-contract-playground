...
const SimpleCompounderModule = buildModule("SimpleCompounderModule", (m) => {
  ...
  const jumpRateModel = m.contract(
    "JumpRateModel",
    [
    ...
    {
      after: [simplePriceOracle],
    }
  );

  const comptroller = m.contract("Comptroller", [], {
    after: [simplePriceOracle, jumpRateModel],
  });

  // 2. 기본 설정만
  const comptrollerSetPriceOracle = m.call(
    comptroller,
    "_setPriceOracle",
    [simplePriceOracle],
    {
  ...
  m.call(comptroller, "_setLiquidationIncentive", ["1080000000000000000"], {
    after: [comptrollerSetPriceOracle],
  }); // 8% 인센티브

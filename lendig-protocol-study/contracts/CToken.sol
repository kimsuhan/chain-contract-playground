...
contract CToken is ERC20, ReentrancyGuard {
    ...
    constructor(
        ...
        reserveFactorMantissa = reserveFactorMantissa_; // 예비 자산 비율 설정
    ...
    function accrueInterest() public returns (uint) {
        ...
        uint totalReservesNew = (reserveFactorMantissa * interestAccumulated) /
            BASE +
            reservesPrior;
    ...
    function borrowInternal(uint borrowAmount) internal returns (uint) {
        ...
        // 대출 잔고 업데이트
        BorrowSnapshot storage borrowSnapshot = accountBorrowSnapshots[
            msg.sender
        ];
